...
class ChatNotifier extends StateNotifier<List<ChatMessage>> {
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      final message = response['message'] as String? ?? 'Sorry, I did not understand that.';
  ...
  Map<String, dynamic> _buildContext() {
    ...
    return {
      ...
      'incomes': currentMonthIncomes.take(20).map((i) => {
        'date': '${i.date.year}-${i.date.month.toString().padLeft(2, '0')}-${i.date.day.toString().padLeft(2, '0')}',
      ...
      'budgets': [
        if (globalBudget != null)
          {'name': 'Monthly', 'limit': globalBudget.limitAmount.toStringAsFixed(2)},
  ...
  Future<String> _createExpense(Map<String, dynamic> data) async {
    try {
      ...
      return '✅ Created expense: ${match.name} \$${amount.toStringAsFixed(2)}';
    } catch (e) {
  ...
  Future<String> _createIncome(Map<String, dynamic> data) async {
    try {
      ...
      await _ref.read(incomesProvider.notifier).addIncome(
        amount: amount,
        source: source,
        date: date,
        note: note,
      ...
      return '✅ Created income: $source \$${amount.toStringAsFixed(2)}';
    } catch (e) {
  ...
  Future<String> _createCategory(Map<String, dynamic> data) async {
    try {
      ...
      await _ref.read(categoriesProvider.notifier).addCategory(
        name: name.trim(),
        iconName: icon,
        color: color,
    ...
    try {
      ...
      final candidates = expenses.where((e) {
        ...
        if (categoryName != null && categoryName.isNotEmpty) {
          ...
          if (cat.name.toLowerCase() != categoryName.toLowerCase().trim()) return false;
        }
        return true;
      }).toList();
      ...
      if (note != null && note.isNotEmpty) {
        final noteMatches = candidates.where(
  ...
  Future<String> _deleteIncome(Map<String, dynamic> data) async {
    try {
      ...
      final candidates = incomes.where((i) {
        ...
        if (dateStr != null && dateStr.isNotEmpty) {
          final d = '${i.date.year}-${i.date.month.toString().padLeft(2, '0')}-${i.date.day.toString().padLeft(2, '0')}';
          if (d != dateStr) return false;
        }
        if (source != null && source.isNotEmpty) {
          if (i.source.toLowerCase() != source.toLowerCase().trim()) return false;
        }
        return true;
      }).toList();
  ...
  Future<String> _deleteCategory(Map<String, dynamic> data) async {
    try {
      ...
      final categories = _ref.read(categoriesProvider);
      final match = _findCategoryByName(categories, name, 'expense') ??
  ...
  static Category? _findCategoryByName(List<Category> categories, String name, String type) {
